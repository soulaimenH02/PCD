...
# ✅ Dossier captures — sauvegarde automatique
import os
SAVE_DIR_OISEAU = "/home/islem/captures/oiseau"
SAVE_DIR_BRUIT  = "/home/islem/captures/bruit"
os.makedirs(SAVE_DIR_OISEAU, exist_ok=True)
os.makedirs(SAVE_DIR_BRUIT,  exist_ok=True)
nb_captures = {"oiseau": 0, "bruit": 0}
...
def predict():
    ...
    try:
        ...
        # ✅ Sauvegarder image avec résultat
        label_cap = resultat['label'].lower()
        nb_captures[label_cap] += 1
        horodatage = time.strftime('%Y%m%d_%H%M%S')
        nom_fichier = f"{horodatage}_{label_cap}_{resultat['confiance']:.0f}pct.jpg"
        if label_cap == "oiseau":
            chemin = os.path.join(SAVE_DIR_OISEAU, nom_fichier)
        else:
            chemin = os.path.join(SAVE_DIR_BRUIT, nom_fichier)
        img_pil = Image.open(BytesIO(image_bytes)).convert("RGB")
        img_pil.save(chemin)
        resultat['capture'] = chemin
        resultat['nb_captures'] = nb_captures.copy()

        symbole = "🐦" if resultat['label'] == "OISEAU" else "🖼️"
        print(f"  {symbole} IMG   {resultat['label']:6s}  "
              f"conf={resultat['confiance']}%  {dt:.0f}ms  → {nom_fichier}")
...
def captures():
    oiseau_files = sorted(os.listdir(SAVE_DIR_OISEAU))
    bruit_files  = sorted(os.listdir(SAVE_DIR_BRUIT))
    return jsonify({
        "nb_oiseau"     : len(oiseau_files),
        "nb_bruit"      : len(bruit_files),
        "total"         : len(oiseau_files) + len(bruit_files),
        "derniers_oiseau": oiseau_files[-5:],
        "derniers_bruit" : bruit_files[-5:],
        "dossier_oiseau" : SAVE_DIR_OISEAU,
        "dossier_bruit"  : SAVE_DIR_BRUIT,
    })
